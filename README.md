...
项目自带 20 题黄金测试集与评分脚本,覆盖四类场景:概览问题 / 精确问题 / 完全无关(应拒答)/ 语义沾边(应拒答):
```bash
cd evals && python rag_test.py
```

当前基线:**检索准确率 100%,回答正确率 92%**

|日期| 参数                                                               |检索准确率|回答准确率|
|---|------------------------------------------------------------------|---|---|
|2026/08/23| similarityThreshold=0.35, topK=20, rerankThreshold=0.1, limit=10 |100%|92%|
